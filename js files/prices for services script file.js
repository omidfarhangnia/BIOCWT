var Dub = [], Jasan = [], DubAndJasan = [];
window.addEventListener("load" , startProcess);
async function startProcess () {
    let DataCollection = {};
    let DubResponse = await fetch("https://raw.githubusercontent.com/omidfarhangnia/BIOCWT-Website/main/js%20files/data/Dub%20wood%20data.json")
    DataCollection.DubData = await DubResponse.json();
    let JasanResponse = await fetch("https://raw.githubusercontent.com/omidfarhangnia/BIOCWT-Website/main/js%20files/data/Jasan%20wood%20data.json");
    DataCollection.JasanData = await JasanResponse.json();
    return DataCollection
}
startProcess().then(function (DataCollection){
    Dub = DataCollection.DubData;
    Jasan = DataCollection.JasanData;
    DubAndJasan = DubAndJasan.concat(Dub , Jasan)
